...
t_ray	generate_ray(t_scene *scene, t_app *app, int x, int y)
{
	...
	ray.direction = vec3_norm(vec3_add(forward, vec3_add(vec3_scale(right, px),
					vec3_scale(up, py))));
	return (ray);
}
...
int	shade_hit(t_scene *scene, t_hit hit, t_vec3 cam_pos)
{
	...
	out_color = vec3_scale(color_mul(obj_color, amb_color),
			scene->ambient.ratio);
	...
	while (light)
	{
		if (!is_in_shadow(scene, hit, light))
		{
			...
			if (ndotl > 0.0)
			{
				...
				out_color = vec3_add(out_color, vec3_scale(color_mul(obj_color,
								light_color), diffuse));
				...
				if (spec > 0.0)
				{
					...
					out_color = vec3_add(out_color, vec3_scale(light_color,
								specular));
				}
			}
		}
...
void	render_scene(t_app *app, t_scene *scene)
{
	...
	while (y < app->height)
	{
		...
		while (x < app->width)
		{
			px = (2.0 * ((x + 0.5) / (double)app->width) - 1.0) * aspect
				* fov_scale;
			py = (1.0 - 2.0 * ((y + 0.5) / (double)app->height)) * fov_scale;
			ray.origin = scene->camera.origin;
